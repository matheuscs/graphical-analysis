...
def find_big_bottom_bodies(df):
    ...
    return pd.DataFrame(big_bottom_bodies, index=big_bottom_bodies_indexes, columns=['Open', 'High', 'Low', 'Close', 'Volume'])
...
def find_big_top_bodies(df):
    ...
    return pd.DataFrame(big_top_bodies, index=big_top_bodies_indexes, columns=['Open', 'High', 'Low', 'Close', 'Volume'])
